...
const parseMatch = (matchData) => {
  try {
    ...
    if (type !== 1 && type !== 6 && type !== 11) {
      console.error(
        `geomatchexporter error: match type ${type} is not supported`,
        matchData,
      );
      return [];
    }
    ...
    if (match.player) {
      ...
      players = match.teams.reduce((acc, team) => acc.concat(team.players), []);
    }
...
const fetchMatchfromJson = (gameMode, gameId) => {
  ...
  switch (gameMode) {
    case "Standard":
      url = `https://www.geoguessr.com/_next/data/${nextBuildId}/en/results/${gameId}.json`;
      break;
    case "Duels":
      url = `https://www.geoguessr.com/_next/data/${nextBuildId}/en/duels/${gameId}/summary.json`;
      break;
    default:
      console.error(
        `geomatchexporter error: gamemode ${gameMode} is not supported (match id: ${gameId})`,
      );
      return null;
  }
...
const fetchMatchfromPage = (gameMode, gameId) => {
  ...
  return fetch(url)
    ...
    .then((html) => {
      ...
      return (
        matchResult.props.pageProps.game ??
        matchResult.props.pageProps.preselectedGame
      );
    })
    .catch((e) => console.error(e));
};
...
const fetchMatchfromEntry = ({ payload }) => {
  if (!nextBuildId) {
    return fetchMatchfromPage(
      payload.gameMode,
      payload.gameToken ?? payload.gameId,
    );
  }

  return fetchMatchfromJson(
    payload.gameMode,
    payload.gameToken ?? payload.gameId,
  );
};
...
const downloadMatchData = async () => {
  ...
  try {
    ...
    const fetchpages = document.querySelector(
      "#geomatchexporter-page-select",
    ).value;
    const batchsize = 40;
    ...
    for (let i = 0; i < matchCount; i += batchsize) {
      ...
      const promises = batch.map(async (entry) => ({
        ...entry,
        match: await fetchMatchfromEntry(entry),
      }));
    ...
    let parsedMatches = matchData.reduce(
      (acc, matchData) => acc.concat(parseMatch(matchData)),
      [],
    );
    parsedMatches = parsedMatches.filter((m) => Boolean(m) && user[m.player]);
    parsedMatches = parsedMatches.map((m) => ({
      ...m,
      player: `${user[m.player]} #${m.player.slice(0, 4)}`,
    }));
